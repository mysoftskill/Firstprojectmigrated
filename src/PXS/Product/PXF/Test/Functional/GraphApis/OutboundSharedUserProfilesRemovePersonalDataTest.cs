...
namespace Microsoft.Membership.MemberServices.PrivacyExperience.FunctionalTests.GraphApis
{
    ...
    /// <summary>
    ///     OutboundSharedUserProfileRemovePersonalDataTest
    /// </summary>
    [TestClass]
    public class OutboundSharedUserProfilesRemovePersonalDataTest : GraphApiTestBase
    {
        ...
        public async Task RemovePersonalDataMissingMsGraphServiceRootShouldBadRequest()
        {
            ...
            using (HttpResponseMessage httpResponseMessage =
                await CallPostAsync(
                    $"directory/outboundSharedUserProfiles('{targetObjectId}')/tenants('{targetTenantId}')/removePersonalData",
        ...
        public async Task RemovePersonalDataShouldSucceed()
        {
            ...
            string targetTenantId = TestData.ResourceTenantId;
            using (HttpResponseMessage httpResponseMessage =
                await CallPostAsync(
                    $"directory/outboundSharedUserProfiles('{targetObjectId}')/tenants('{targetTenantId}')/removePersonalData",
                    TestIdentityHomeTenant,
        ...
        [TestMethod, TestCategory("FCT"), TestCategory("PROXY")]
        public async Task RemovePersonalDataTargetTenantIdNotGuidShouldBadRequest()
        {
            Guid targetObjectId = Guid.NewGuid();
            string targetTenantId = "not-a-guid";
            using (HttpResponseMessage httpResponseMessage =
                await CallPostAsync(
                    $"directory/outboundSharedUserProfiles('{targetObjectId}')/tenants('{targetTenantId}')/removePersonalData", 
                    TestIdentityHomeTenant,
                    new EmptyBody { }).ConfigureAwait(false))
            {
                ...
                Assert.IsNotNull(odataError.Error, responseDetails);
                Assert.AreEqual(ErrorCode.InvalidInput.ToString(), odataError.Error.Code, responseDetails);
                Assert.AreEqual(string.Format(GraphApiErrorMessage.InvalidTenantIdFormat, targetTenantId), odataError.Error.Message, responseDetails);
        ...
        [TestMethod, TestCategory("FCT"), TestCategory("PROXY")]
        public async Task OutboundRemovePersonalDataInResourceTenantShouldFail()
        {
            string targetObjectId = TestData.ResourceUserObjIdNonComplexOrg;
            string targetTenantId = TestData.ResourceTenantId;
            using (HttpResponseMessage httpResponseMessage =
                await CallPostAsync(
                    $"directory/outboundSharedUserProfiles('{targetObjectId}')/tenants('{targetTenantId}')/removePersonalData",
        ...
        public async Task OutboundRemovePersonalDataShouldRelayCorrectErrorCodeForRVSFailure(int statusCode)
        {
            using (HttpResponseMessage httpResponseMessage =
            await CallPostAsync(
                $"directory/outboundSharedUserProfiles('{TestAadRvsErrorObjectIds[statusCode].TargetObject}')/tenants('{TestData.ResourceTenantId}')/removePersonalData",
                TestIdentityHomeTenant,
                new EmptyBody { }).ConfigureAwait(false))
            {
                Assert.IsNotNull(httpResponseMessage);
