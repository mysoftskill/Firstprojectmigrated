...
namespace Microsoft.Membership.MemberServices.Privacy.AqsWorker.UnitTests.AzureQueueProcessor
{
    ...
    /// <summary>
    ///     MsaAccountDeleteQueueTest
    /// </summary>
    [TestClass]
    public class MsaAccountDeleteQueueTest
    {
        private readonly ILogger logger = new ConsoleLogger();

        private readonly Mock<IMsaUserDeleteQueueConfiguration> mockConfig = CreateMockMsaUserDeleteQueueConfig();
        ...
        {
            var manager = new MsaAccountDeleteQueue(this.mockQueueStorageProviders.Select(c => c.Object).ToList(), this.logger, this.mockConfig.Object, this.mockCounterFactory.Object);
            try
            {
        ...
        public async Task ShouldDequeueFromAllQueuesWithNoMessagesReturned(int numberOfQueues)
        {
            ...
            for (int i = 0; i < numberOfQueues; i++)
            {
                ...
                mockQueueStorageProvider.Setup(c => c.GetCloudQueueAsync(nameof(AccountDeleteInformation).ToLowerInvariant(), It.IsAny<CancellationToken>())).ReturnsAsync(mockQueue.Object);
                this.mockQueueStorageProviders.Add(mockQueueStorageProvider);
            }
        ...
        public void ShouldThrowIfNoStorageProvidersSpecified()
        {
            try
            {
                var manager = new MsaAccountDeleteQueue(new List<IAzureStorageProvider>(), this.logger, this.mockConfig.Object, this.mockCounterFactory.Object);
                Assert.Fail("Should have thrown");
            }
            catch (ArgumentOutOfRangeException e)
            {
                StringAssert.StartsWith(
                    value: e.Message,
                    substring: "Must initialize with at least 1 queue. There were 0.");
                throw;
            }
        }
        ...
        public async Task ShouldEnqueueSuccessWithMaxTtl()
        {
            ...
            await manager.EnqueueAsync(new List<AccountDeleteInformation> { new AccountDeleteInformation() }, CancellationToken.None).ConfigureAwait(false);
        ...
        public void CreateNewManager(IList<IAzureStorageProvider> queueStorageProviders, IMsaUserDeleteQueueConfiguration config, ICounterFactory counterFactory)
        {
            new MsaAccountDeleteQueue(
        ...
        protected static Mock<IMsaUserDeleteQueueConfiguration> CreateMockMsaUserDeleteQueueConfig()
        {
            return new Mock<IMsaUserDeleteQueueConfiguration>();
        }
        ...
        private static IEnumerable<object[]> CreateAadAccountCloseQueueManagerTestData()
        {
            ...
            var aadAccountCloseConfig = CreateMockMsaUserDeleteQueueConfig();
