...
namespace Microsoft.Membership.MemberServices.Privacy.VortexDeviceDeleteWorker
{
    ...
    using Microsoft.Membership.MemberServices.Privacy.VortexDeviceDeleteWorker.Host;
    ...
    {
        private const string OperationName = "VortexDeviceDelete";
        ...
        internal static void Main()
        {
            ...
            HostDecorator vortexQueueProcessor = new VortexQueueProcessorDecorator(dependencies);
            HostDecorator aidQueueProcessor = new AidQueueProcessorDecorator(dependencies);            

            HostDecorator removeConsoleLoggerDecorator = new RemoveConsoleLoggerDecorator(
                dependencies.Container.Resolve<IPrivacyConfigurationManager>().VortexDeviceDeleteWorkerConfiguration.EnableConsoleLogging);

            try
            {
                SetupSllOperationName(OperationName);
                IHost serviceHost = HostFactory.CreatePipeline(
                    vortexQueueProcessor,
                    aidQueueProcessor,
