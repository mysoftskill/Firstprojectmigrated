...
namespace Microsoft.Membership.MemberServices.Privacy.AqsWorker.AzureQueueProcessor
{
    ...
    /// <inheritdoc />
    /// <summary>
    ///     Writes <see cref="AccountDeleteInformation"/> to Azure Queue
    /// </summary>
    public class AccountDeleteQueueWriter : IAccountDeleteWriter
    {
        ...
        public AccountDeleteQueueWriter(
        ...
        public async Task<AdapterResponse<IList<AccountDeleteInformation>>> WriteDeletesAsync(IList<AccountDeleteInformation> accountDeleteInfos, string requester)
        {
            ...
            try
            {
                await this.queue.EnqueueAsync(accountDeleteInfos, CancellationToken.None).ConfigureAwait(false);
            }
            catch (Exception e)
            {
                var errorMessage = $"Write to azure queue failed for {nameof(AccountDeleteInformation)}";
                this.logger.Error(nameof(AccountDeleteQueueWriter), e, errorMessage);
                return new AdapterResponse<IList<AccountDeleteInformation>> { Error = new AdapterError(AdapterErrorCode.Unknown, errorMessage, 500) };
            }

            // Update stats on what was written.
