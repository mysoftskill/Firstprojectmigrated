...
namespace Microsoft.PrivacyServices.AzureKeyVaultCertificateInstaller
{
    ...
    public static class NetworkShellHttpContext
    {
        ...
        public static void AddSslCert(string ipAddress, string port, string appId, string certHash)
        {
            try
            {
                CommandPrompt.Execute(
                    "netsh http add sslcert ipport={0}:{1} appid={2} certhash={3}".FormatInvariant(ipAddress, port, appId, certHash));
            }
            catch (CommandPromptRequestException requestException)
            {
                ...
                RepairCertificate(StoreName.My, certHash);
                CommandPrompt.Execute(
                    "netsh http add sslcert ipport={0}:{1} appid={2} certhash={3}".FormatInvariant(ipAddress, port, appId, certHash));
            }
        }
        ...
        public static void DeleteSslCert(string ipAddress, string port)
        {
            try
            {
                CommandPrompt.Execute("netsh http delete sslcert ipport={0}:{1}".FormatInvariant(ipAddress, port));
            }
            catch (CommandPromptRequestException requestException)
            {
        ...
        private static int ParseNetshErrorCodeOutput(string output)
        {
            ...
            if (!match.Success)
            {
                throw new ArgumentException("Provided output did not contain an error code. Output: {0}".FormatInvariant(output), nameof(output));
            }
        ...
        private static void RepairCertificate(StoreName storeName, string certHash)
        {
            try
            {
                CommandPrompt.Execute("certutil -repairstore {0} {1}".FormatInvariant(storeName, certHash));
            }
