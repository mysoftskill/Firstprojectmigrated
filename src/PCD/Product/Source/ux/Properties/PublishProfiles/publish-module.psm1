...
function Publish-AspNetMSDeploy{
    ...
    process{
        if($publishProperties){
            ...
            # <dbFullSql path="Data Source=tcp:webapplicationdbserver.database.windows.net,1433;Initial Catalog=WebApplication_db;@@@" />
            # <dbFullSql path="Data Source=tcp:webapplicationdbserver.database.windows.net,1433;Initial Catalog=WebApplication_db;@@@" />
            # </sitemanifest>
            
            # This example was triggering issues with the credentials scan. Remember to make sure to replace @@@ with a user id and password
    ...
    process{
        ...
        if(-not [string]::IsNullOrWhiteSpace($serviceUrl)){
            if([string]::Compare($serviceMethod,'WMSVC',[StringComparison]::OrdinalIgnoreCase) -eq 0){
                ...
                if(-not ($serviceUrl.StartsWith($httpsStr,[StringComparison]::OrdinalIgnoreCase)) ){
            ...
            elseif([string]::Compare($serviceMethod,'RemoteAgent',[StringComparison]::OrdinalIgnoreCase) -eq 0){
                [System.UriBuilder]$serviceUriBuilder = New-Object -TypeName 'System.UriBuilder' $serviceUrl
                # https://{computername}/MSDEPLOYAGENTSERVICE
