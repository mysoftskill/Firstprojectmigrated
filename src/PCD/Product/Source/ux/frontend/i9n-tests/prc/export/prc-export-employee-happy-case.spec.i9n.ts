...
describe("On manual requests Export page", () => {
    beforeAll(() => {
        Test.Navigation.loadPage("manual-requests/export", "manual-requests.export.employee");
    });

    it("user should see the Export request form elements(3rd call)", () => {
        Test.Verify.SharedComponent.verifyManualExportRequestForm();
    });

    describe("when user enters information for Microsoft employee and clicks Export button", () => {
