...
function popFromQueue(minCk, maxCk, updateCk, updateNvt, maxItems, partitionKey) {
    ...
    var query = {
        query: 'SELECT TOP @maxItems * FROM c WHERE (c.ck BETWEEN @minCk AND @maxCk) AND c.pk = @partitionKey ORDER BY c.ck ASC',
        parameters: [
