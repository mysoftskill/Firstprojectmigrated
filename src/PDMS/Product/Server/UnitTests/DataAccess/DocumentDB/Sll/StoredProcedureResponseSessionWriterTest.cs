namespace Microsoft.PrivacyServices.DataManagement.DataAccess.DocumentDb.UnitTest
{
    ...
    public class StoredProcedureResponseSessionWriterTest
    {
        [Theory(DisplayName = "Verify StoredProcedureResponse sll event."), AutoMoqData]
        public void VerifyConversion(Mock<ILogger<Base>> log, TestObject value, string activityId, double requestCharge, string uri)
        {
            ...
            var docs = DocumentModule.Create(value);
            var response = StoredProcedureResponseModule.Create<Document[]>(new[] { docs }, headers);

            var data = new Tuple<string, StoredProcedureResponse<Document[]>>(uri, response);
