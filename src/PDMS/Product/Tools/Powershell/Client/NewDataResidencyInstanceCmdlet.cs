...
namespace Microsoft.PrivacyServices.DataManagement.Client.Powershell
{
    ...
    /// <summary>
    /// A <c>cmdlet</c> for creating DataResidencyBoundaryIds.
    /// </summary>
    [Cmdlet(VerbsCommon.New, "PdmsDataResidencyInstance")]
    public class NewDataResidencyInstanceCmdlet : Cmdlet
    {
        /// <summary>
        /// The cloud instance string.
        /// </summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            {
                this.WriteObject(this.Value.Select(Policies.Current.DataResidencyInstances.CreateId).ToArray());
            }
            else
            {
                foreach (var value in this.Value)
                {
                    this.WriteObject(Policies.Current.DataResidencyInstances.CreateId(value));
                }
            }
        }
    }
