...
namespace Microsoft.PrivacyServices.DataManagement.Client.Powershell
{
    ...
    /// <summary>
    /// A <c>cmdlet</c> for creating ProtocolIds.
    /// </summary>
    [Cmdlet(VerbsCommon.New, "PdmsProtocol")]
    public class NewProtocolCmdlet : Cmdlet
    {
        /// <summary>
        /// The protocol string.
        /// </summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            {
                this.WriteObject(this.Value.Select(Policies.Current.Protocols.CreateId).ToArray());
            }
            else
            {
                foreach (var value in this.Value)
                {
                    this.WriteObject(Policies.Current.Protocols.CreateId(value));
                }
            }
        }
    }
