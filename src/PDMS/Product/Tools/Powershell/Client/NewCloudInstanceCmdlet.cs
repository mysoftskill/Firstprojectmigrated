...
namespace Microsoft.PrivacyServices.DataManagement.Client.Powershell
{
    ...
    /// <summary>
    /// A <c>cmdlet</c> for creating CloudInstanceIds.
    /// </summary>
    [Cmdlet(VerbsCommon.New, "PdmsCloudInstance")]
    public class NewCloudInstanceCmdlet : Cmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            {
                this.WriteObject(this.Value.Select(Policies.Current.CloudInstances.CreateId).ToArray());
            }
            else
            {
                foreach (var value in this.Value)
                {
                    this.WriteObject(Policies.Current.CloudInstances.CreateId(value));
                }
            }
        }
    }
