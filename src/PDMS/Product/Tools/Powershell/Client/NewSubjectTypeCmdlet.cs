...
namespace Microsoft.PrivacyServices.DataManagement.Client.Powershell
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "PdmsSubjectType")]
    public class NewSubjectTypeCmdlet : Cmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            {
                this.WriteObject(this.Value.Select(Policies.Current.SubjectTypes.CreateId).ToArray());
            }
            else
            {
                foreach (var value in this.Value)
                {
                    this.WriteObject(Policies.Current.SubjectTypes.CreateId(value));
                }
            }
        }
    }
