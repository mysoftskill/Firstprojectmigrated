namespace Microsoft.PrivacyServices.DataManagement.Client
{
    ...
    public class EnumTolerantConverter<TEnum> : StringEnumConverter where TEnum : struct
    {
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            if (reader.TokenType == JsonToken.String)
            {
                ...
                if (EnumNameMapping.Value.TryGetValue(name, out TEnum result))
                {
                    return result;
                }
            ...
            if (typeof(Nullable<TEnum>) == objectType)
            {
                return null;
            }
            else
